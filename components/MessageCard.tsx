...
const MessageCard: React.FC<MessageCardProps> = ({ message, isLoading, isSaved, onSave, onRefresh }) => {
  ...
  if (isLoading || !message) {
    return (
      <div className="flex flex-col items-center justify-center space-y-8 animate-pulse h-64 md:h-80 w-full max-w-lg">
        <div className="h-4 w-32 bg-[#4a4e69]/5 rounded-full mb-4"></div>
        <div className="h-10 w-full bg-[#4a4e69]/5 rounded-xl"></div>
        <div className="h-10 w-3/4 bg-[#4a4e69]/5 rounded-xl"></div>
        <div className="h-4 w-40 bg-[#4a4e69]/5 rounded-full mt-4"></div>
      </div>
    );
  }
  ...
  return (
    <div 
      className="w-full max-w-3xl px-8 py-12 md:py-20 text-center animate-fade-in flex flex-col items-center relative select-none"
    >
      <div className="absolute right-0 top-0 md:-right-4 md:top-4 flex flex-col space-y-4 z-10 opacity-30 hover:opacity-100 transition-opacity duration-500">
        <button 
          ...
          title={isSaved ? "Remove" : "Save to Favorites"}
        >
          <svg 
      ...
      <div className="text-3xl md:text-5xl lg:text-6xl leading-[1.3] serif italic text-[#22223b] mb-10 md:mb-14">
      ...
      {message.author && (
        <p className="text-xs md:text-sm uppercase tracking-[0.5em] text-[#9a8c98] font-light opacity-60 mb-20">
          — {message.author}
        </p>
      )}
      ...
      <div className="flex flex-col items-center group">
        <button 
          onClick={onRefresh}
          className="relative w-16 h-16 rounded-full bg-white/40 border border-[#f2e9e4] flex items-center justify-center text-[#9a8c98] transition-all duration-700 hover:text-[#4a4e69] hover:bg-white hover:shadow-2xl hover:scale-110 active:scale-95 focus:outline-none"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" className="group-hover:rotate-180 transition-transform duration-1000">
            <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
          </svg>
        </button>
        <span className="mt-4 text-[10px] uppercase tracking-[0.4em] text-[#9a8c98] opacity-0 group-hover:opacity-100 transition-all duration-700 translate-y-2 group-hover:translate-y-0">
          Kindle another
        </span>
      </div>

      {message.type === 'compliment' && (
        <div className="absolute -top-6 left-1/2 -translate-x-1/2 px-6 py-2 shimmer-bg text-[#4a4e69] text-[10px] font-medium rounded-full uppercase tracking-[0.4em] shadow-sm border border-[#f2e9e4]">
          Warmth
        </div>
      )}
    </div>
